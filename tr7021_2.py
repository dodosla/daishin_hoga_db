...
class Form(QtWidgets.QDialog):
    def __init__(self, parent=None):
        ...
        self.DCth = DataCheckThread(self)
        self.DCth.threadEvent.connect(self.DataCheckThreadEventHandler)
        self.DCth.start()

        self.con_hoga = sqlite3.connect("hoga.db")
        self.stock_df = pd.DataFrame(columns=['YYYYMMDD', 'HHMMSS', 'cur', 'diff', 'diffp',
        ...
        self.SaveDone = 0
        self.SaveStartDate = 8_40_00  # 장시작시간 ?
        self.SaveEndDate = 15_31_00 #장마감시간 ?
    ...
    def DataCheckThreadEventHandler(self):
        now = datetime.now()
        YYYYMMDD = (now.year * 100 * 100) + (now.month * 100) + now.day
        HHMMSS = (now.hour * 100 * 100) + (now.minute * 100) + (now.second)
        print("DataCheckThreadEventHandler called", HHMMSS)
        if(self.SaveStartDate <= HHMMSS <= self.SaveEndDate):
            new_list = [(YYYYMMDD, HHMMSS, self.item.cur, self.item.diff, self.item.diffp,
            ...
            new_df = pd.DataFrame(new_list, columns=['YYYYMMDD', 'HHMMSS', 'cur', 'diff', 'diffp',
            'offer[0]', 'offer[1]', 'offer[2]', 'offer[3]', 'offer[4]', 'offer[5]', 'offer[6]', 'offer[7]', 'offer[8]', 'offer[9]',
            'bid[0]', 'bid[1]', 'bid[2]', 'bid[3]', 'bid[4]', 'bid[5]', 'bid[6]', 'bid[7]', 'bid[8]', 'bid[9]',
            'offervol[0]', 'offervol[1]', 'offervol[2]', 'offervol[3]', 'offervol[4]', 'offervol[5]', 'offervol[6]', 'offervol[7]', 'offervol[8]', 'offervol[9]',
            'bidvol[0]', 'bidvol[1]', 'bidvol[2]', 'bidvol[3]', 'bidvol[4]', 'bidvol[5]', 'bidvol[6]', 'bidvol[7]', 'bidvol[8]', 'bidvol[9]',
            ...
            if(self.SaveDone == 0) and (self.SaveEndDate < HHMMSS):
                self.stock_df.to_sql("000660", self.con_hoga, if_exists='append', index_label='date')
                self.SaveDone = 1
                exit()
